...
from DataClasses.PrekeyBundle import PreKeyBundle
from DataClasses.InitialMessage import InitialMessage
...
class X3DH:
    ...
    def __init__(self, N: int = 10) -> None:
        ...
        self.identity_private_key, self.identity_public_key = X3DH.__generate_identity_keypair()
        ...
        ) = X3DH.__generate_signed_prekey(self.signing_private_key)
        # Generate a one-time pre-key
        self.one_time_prekey_public_list, self.one_time_prekey_private_list = X3DH.__generate_one_time_prekey_list(N)
    ...
    def initiate_handshake(self, peer_prekey_bundle: PreKeyBundle, one_time_prekey_public_index: int = 0) -> Tuple[InitialMessage, bytes, Optional[X25519PrivateKey]]:
        ...
        shared_secret = X3DH.__derive_shared_secret(
    ...
    def respond_handshake(self, initial_message: InitialMessage) -> bytes:
        ...
        # Derive the final shared secret
        return X3DH.__derive_shared_secret(
            [dh_signed_identity, dh_identity_ephemeral, dh_signed_ephemeral, dh_one_time_ephemeral],
            salt_material
        )

    def serialize_x3dh(self) -> str:
        """
        Serialize the X3DH instance to a JSON string.
        """
        ...
        identity_pub = self.identity_public_key.public_bytes(
            encoding=Encoding.Raw,
        ...
        signing_pub = self.signing_public_key.public_bytes(
            encoding=Encoding.Raw,
            format=PublicFormat.Raw
        )
        # Signed pre-key
        ...
        spk_pub = self.signed_prekey_public_key.public_bytes(
            encoding=Encoding.Raw,
            format=PublicFormat.Raw
        )
        ...
        one_time_prekey_private_bytes_list = [
            ...
            for key in self.one_time_prekey_private_list
        ]
        ot_pub_list = [
            key.public_bytes(
                encoding=Encoding.Raw,
                format=PublicFormat.Raw
            ) for key in self.one_time_prekey_public_list
        ]
    ...
    @staticmethod
    def deserialize_x3dh(serialized: str) -> 'X3DH':
        """
        Deserialize a JSON string back into an X3DH instance.
        """
        ...
        inst = X3DH(N=data['N'])
        # Override generated keys with loaded
        inst.identity_private_key = X25519PrivateKey.from_private_bytes(from_b64(data['identity_private_key']))
        inst.identity_public_key = X25519PublicKey.from_public_bytes(from_b64(data['identity_public_key']))
        inst.signing_private_key = Ed25519PrivateKey.from_private_bytes(from_b64(data['signing_private_key']))
        inst.signing_public_key = Ed25519PublicKey.from_public_bytes(from_b64(data['signing_public_key']))
        inst.signed_prekey_private_key = X25519PrivateKey.from_private_bytes(from_b64(data['signed_prekey_private_key']))
        inst.signed_prekey_public_key = X25519PublicKey.from_public_bytes(from_b64(data['signed_prekey_public_key']))
        inst.signed_prekey_signature = from_b64(data['signed_prekey_signature'])
        inst.one_time_prekey_private_list = [
            X25519PrivateKey.from_private_bytes(from_b64(p)) if p is not None else None
            for p in data['one_time_prekey_private_list']
        ]
        inst.one_time_prekey_public_list = [
            X25519PublicKey.from_public_bytes(from_b64(p))
            for p in data['one_time_prekey_public_list']
        ]
        return inst
    ...
    @staticmethod
    def __generate_identity_keypair() -> Tuple[X25519PrivateKey, X25519PublicKey]:
        """
    ...
    @staticmethod
    def __generate_signed_prekey(signing_key_private: Ed25519PrivateKey) -> Tuple[X25519PrivateKey, X25519PublicKey, bytes]:
        """
    ...
    @staticmethod
    def __generate_one_time_prekey_list(N: int) -> Tuple[list[X25519PublicKey], list[X25519PrivateKey]]:
        """
    ...
    @staticmethod
    def __derive_shared_secret(dh_shared_materials: List[bytes], salt: bytes, info: bytes = b'X3DH') -> bytes:
        """
