...
class Server:
    ...
    def register_server(self, admin_username: str, admin_password: str, verbose: bool = False):
        ...
        database_path = Server.database_dir + Server.__hash(admin_username) + ".db"
        ...
        self.database = ServerDatabase(database_dir=Server.database_dir, db_name=Server.__hash(admin_username), admin_password_hashed=password_hashed, aes_cipher=aes_cipher,
                                       verbose=verbose)
    ...
    def __create_account(self, phone_hashed: str, prekey_bundle_serialized: bytes) -> bool:
        ...
        if self.database.phone_hashed_in_users(phone_hashed):
            return False
        return self.database.save_user_prekey_bundle_serialized(phone_hashed, prekey_bundle_serialized)

    def __delete_account(self, phone_hashed: str) -> bool:
        if not self.database.phone_hashed_in_users(phone_hashed):
            return False
        return self.database.delete_account(phone_hashed)
    ...
    def __save_message_on_server(self, message_serialized: bytes) -> bool:
        ...
        return self.database.save_message_serialized(receiver, message_serialized)

    def __fetch_my_messages(self, receiver_phone_hashed: str) -> tuple[list[bytes], bool]:
        """
        ...
        success_ = True
        if messages_serialized is None:
            success_ = False
            messages_serialized = []

        if self.verbose:
            self._debug_list_messages()
        return messages_serialized, success_

    def __analyze_request(self, request: tuple) -> tuple[(bytes | list[bytes] | str), bool]:
        """
        ...
        if command == "register":
            ...
            return ("Account created successfully!".encode("utf-8"), success) if success else ("Error: account already exists!".encode("utf-8"), success)

        elif command == "delete":
            phone_hashed = request[1]
            success = self.__delete_account(phone_hashed)
            if self.verbose and success:
                print(f"{phone_hashed} is deleted from the server.")
            return ("Account deleted successfully!".encode("utf-8"), success) if success else ("Error: account does not exist!".encode("utf-8"), success)
            ...
            if prekey_bundle_serialized is None:
                return "Error: no account found!".encode("utf-8"), False
            return prekey_bundle_serialized, True

        elif command in ("initial_message", "private_message"):
            # Store incoming PrivateMessage on server queue
            message_serialized = request[1]
            success = self.__save_message_on_server(message_serialized)
            if self.verbose:
                self._debug_list_messages()
            return (("Double Ratchet initial message stored on server.".encode("utf-8"), success)
                    if command == "initial_message"
                    else ("Message stored on server.".encode("utf-8"), success))
            ...
            return "Error: invalid message!".encode("utf-8"), False
                ...
                try:
                    with conn:
                        while True:
                            ...
                            response, success = self.__analyze_request(request)
                            out = pickle.dumps(response)
                            conn.sendall(len(out).to_bytes(4, 'big') + success.to_bytes(4, 'big') + out)
    ...
    @staticmethod
    def delete_server(admin_username: str) -> bool:
        database_path = Server.database_dir + Server.__hash(admin_username) + ".db"
        if os.path.exists(database_path):
